...
public class Screen {
	...
	public int avgHex(int c1, int c2) {
		String s = Integer.toHexString(c1);
		String c = Integer.toHexString(c2);
		return Integer.parseInt(String
				.valueOf((Integer.parseInt(s.substring(2, 4), 16) + Integer.parseInt(c.substring(2, 4), 16)) / 4)
				+ String.valueOf(
						(Integer.parseInt(s.substring(4, 6), 16) + Integer.parseInt(c.substring(4, 6), 16)) / 4)
				+ String.valueOf(
						(Integer.parseInt(s.substring(6, 8), 16) + Integer.parseInt(c.substring(6, 8), 16)) / 4),
				16);
	}
	...
	public void drawTexture(int x, int y, Texture texture, boolean flipX) {
		...
		for (int yy = 0; yy < texture.getHeight(); yy++) {
			...
			for (int xx = 0; xx < texture.getWidth(); xx++) {
				...
				if (color == 0xff00ffff) {
					//pixels[xo + yo * this.width] = avgHex(pixels[xo + yo * this.width], shade);
					continue;
				}
